...
class TestGitWorktreeManager:
    ...
    @pytest.mark.asyncio
    async def test_create_worktree_branch_delete_fails(self):
        mgr = GitWorktreeManager("/tmp/repo")
        mgr._run_git = AsyncMock(side_effect=[WorktreeError("no branch"), None, None, None, None])
        result = await mgr.create_worktree("feature-x", "main")
    ...
    @pytest.mark.asyncio
    async def test_remove_worktree_removal_fails(self):
        mgr = GitWorktreeManager("/tmp/repo")
        mgr._run_git = AsyncMock(
            side_effect=[WorktreeError("no worktree"), None]
        )
        await mgr.remove_worktree("feature-x")
        assert mgr._run_git.call_count == 2

    @pytest.mark.asyncio
    async def test_remove_worktree_both_fail(self):
        mgr = GitWorktreeManager("/tmp/repo")
        mgr._run_git = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_rebase_to_main(self):
        mgr = GitWorktreeManager()
        mgr._run_git = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_list_worktrees_empty(self):
        mgr = GitWorktreeManager()
        mgr._run_git = AsyncMock(return_value="")
        result = await mgr.list_worktrees()
        assert result == []

    @pytest.mark.asyncio
    async def test_get_status_with_changes(self):
        mgr = GitWorktreeManager()
    ...
    @pytest.mark.asyncio
    async def test_get_status_without_changes(self):
        mgr = GitWorktreeManager()
        mgr._run_git = AsyncMock(side_effect=["", "main"])
        result = await mgr.get_status("/tmp/repo-feature-x")
        assert result == {
            "branch": "main",
            "has_changes": False,
    ...
    async def test_get_status_error(self):
        mgr = GitWorktreeManager()
        mgr._run_git = AsyncMock(side_effect=WorktreeError("fatal: not a git repository"))
        result = await mgr.get_status("/tmp/repo-feature-x")
        assert result == {"error": "fatal: not a git repository"}

    @pytest.mark.asyncio
    async def test_get_singleton_default(self):
    ...
    @pytest.mark.asyncio
    async def test_get_singleton_custom_path(self):
        import app.git_worktree as gw_module
