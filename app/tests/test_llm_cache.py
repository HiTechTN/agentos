...
class TestPersistentLLMCache:
    ...
    @pytest.mark.asyncio
    async def test_connect_redis_unavailable_keeps_l1_only(self):
    ...
    @pytest.mark.asyncio
    async def test_l1_populated_on_set(self):
        cache = PersistentLLMCache()
    ...
    @pytest.mark.asyncio
    async def test_invalidate_pattern_no_redis_returns_zero(self):
        cache = PersistentLLMCache()
        count = await cache.invalidate_pattern("llm_cache:*")
        assert count == 0

    @pytest.mark.asyncio
    async def test_invalidate_pattern_with_redis(self):
        cache = PersistentLLMCache()
    ...
    @pytest.mark.asyncio
    async def test_singleton_instance_is_same(self):
