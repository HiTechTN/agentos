...
class TestEmbeddingClient:
    @pytest.mark.asyncio
    async def test_get_openai_client_creates_client(self):
    ...
    @pytest.mark.asyncio
    async def test_get_openai_client_returns_cached(self):
        from app.utils.api_clients import EmbeddingClient
        client = EmbeddingClient()
        fake = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_embed_fallback_to_ollama_success(self):
        from app.utils.api_clients import EmbeddingClient
        client = EmbeddingClient()
    ...
    @pytest.mark.asyncio
    async def test_embed_all_providers_fail_returns_zero_vector(self):
        from app.utils.api_clients import EmbeddingClient
        client = EmbeddingClient()
    ...
    @pytest.mark.asyncio
    async def test_fallback_ollama_embed_success(self):
        from app.utils.api_clients import EmbeddingClient
        client = EmbeddingClient()
    ...
    @pytest.mark.asyncio
    async def test_fallback_ollama_embed_failure_returns_zero_vector(self):
        from app.utils.api_clients import EmbeddingClient
        client = EmbeddingClient()
...
class TestLLMClient:
    ...
    async def test_fallback_ollama_success(self):
        ...
        mock_response.json = MagicMock(
            return_value={"message": {"content": "Ollama reply"}}
        )
    ...
    @pytest.mark.asyncio
    async def test_fallback_ollama_failure_raises_llm_unavailable(self):
    ...
    def test_get_openai_client_llm_creates_and_caches(self):
        from app.utils.api_clients import LLMClient
        settings = get_settings()
        client = LLMClient()
        assert client._openai_client is None
    ...
    @pytest.mark.asyncio
    async def test_chat_with_tools_does_not_cache(self):
        from app.utils.api_clients import LLMClient
        client = LLMClient()
        client._llm_cache.clear()
    ...
    async def test_chat_fallback_on_error(self):
        ...
        mock_response.raise_for_status = MagicMock()
        mock_response.json = MagicMock(
            return_value={"message": {"content": "Ollama fallback"}}
        )
        ...
        with patch("app.utils.api_clients.httpx.AsyncClient", return_value=mock_client):
            result = await client.chat("gpt-4", [{"role": "user", "content": "hi"}], temperature=0.5)  # noqa: E501
    ...
    @pytest.mark.asyncio
    async def test_chat_cache_disabled_does_not_cache(self):
        from app.utils.api_clients import LLMClient
        settings = get_settings()
...
class TestNotificationManager:
    ...
    @pytest.mark.asyncio
    async def test_send_handler_exception_returns_false(self):
        from app.utils.notifications import NotificationManager
        nm = NotificationManager()
        nm._send_console = AsyncMock(side_effect=Exception("crash"))
        result = await nm.send("console", "Title", "Msg")
        assert result is False

    @pytest.mark.asyncio
    async def test_send_slack_with_details(self):
        from app.utils.notifications import NotificationManager
        settings = get_settings()
    ...
    @pytest.mark.asyncio
    async def test_send_slack_no_webhook_returns_early(self):
        from app.utils.notifications import NotificationManager
        settings = get_settings()
    ...
    @pytest.mark.asyncio
    async def test_send_discord_with_webhook(self):
        from app.utils.notifications import NotificationManager
        settings = get_settings()
    ...
    @pytest.mark.asyncio
    async def test_send_discord_no_webhook_returns_early(self):
        from app.utils.notifications import NotificationManager
        settings = get_settings()
    ...
    @pytest.mark.asyncio
    async def test_notify_all_without_slack(self):
        from app.utils.notifications import NotificationManager
        nm = NotificationManager()
        result = await nm.notify_all("Global", "announcement")
        assert result is None

    @pytest.mark.asyncio
    async def test_notify_all_with_slack(self):
        from app.utils.notifications import NotificationManager
        settings = get_settings()
        settings.slack_webhook_url = "https://hooks.slack.com/test"
        nm = NotificationManager()
        ...
        with patch("app.utils.notifications.httpx.AsyncClient", return_value=mock_client):
            result = await nm.notify_all("Global", "announcement", details={"version": "5.0"})
        assert result is None

    def test_get_notifications_singleton(self):
...
class TestSubAgent:
    ...
    def test_get_sub_agent_custom_config(self):
        from app.agents.sub_agent import get_sub_agent
        fake_yaml = """---
name: CustomAgent
model: openai/gpt-4
...
"""
        with (
            patch("app.agents.sub_agent.os.path.expanduser", return_value="/fake/.agentos/subagents/custom.md"),  # noqa: E501
            patch("app.agents.sub_agent.os.path.exists", return_value=True),
            patch("builtins.open", MagicMock(return_value=MagicMock(
                __enter__=MagicMock(return_value=MagicMock(read=MagicMock(return_value=fake_yaml)))
            ))),
        ):
    ...
    def test_get_sub_agent_not_found(self):
        from app.agents.sub_agent import get_sub_agent
        with (
            patch("app.agents.sub_agent.os.path.expanduser", return_value="/fake/.agentos/subagents/nope.md"),  # noqa: E501
            patch("app.agents.sub_agent.os.path.exists", return_value=False),
        ):
            agent = get_sub_agent("nope")
        assert agent is None

    def test_load_custom_config_success(self):
...
"""
    ...
    def test_load_custom_config_no_frontmatter(self):
        from app.agents.sub_agent import _load_custom_config
        fake_file = MagicMock()
        fake_file.__enter__.return_value.read.return_value = "Just a plain markdown file without frontmatter."  # noqa: E501
        with patch("builtins.open", return_value=fake_file):
    ...
    def test_load_custom_config_invalid_yaml(self):
        from app.agents.sub_agent import _load_custom_config
        fake_content = """---
name: TestAgent
...
"""
    ...
    def test_get_or_create_sub_agent_new(self):
    ...
    def test_get_or_create_sub_agent_cached(self):
        from app.agents.sub_agent import get_or_create_sub_agent, sub_agent_registry
        sub_agent_registry.clear()
    ...
    def test_get_or_create_sub_agent_not_found(self):
        from app.agents.sub_agent import get_or_create_sub_agent, sub_agent_registry
        sub_agent_registry.clear()
        with (
            patch("app.agents.sub_agent.os.path.expanduser", return_value="/fake/.agentos/subagents/unknown.md"),  # noqa: E501
            patch("app.agents.sub_agent.os.path.exists", return_value=False),
        ):
            agent = get_or_create_sub_agent("unknown")
        assert agent is None

    def test_route_to_sub_agent_verifier(self):
    ...
    def test_route_to_sub_agent_explorer(self):
        from app.agents.sub_agent import route_to_sub_agent
    ...
    def test_route_to_sub_agent_code_reviewer(self):
        from app.agents.sub_agent import route_to_sub_agent
    ...
    def test_route_to_sub_agent_planner(self):
        from app.agents.sub_agent import route_to_sub_agent
    ...
    def test_route_to_sub_agent_debugger(self):
        from app.agents.sub_agent import route_to_sub_agent
    ...
    @pytest.mark.asyncio
    async def test_sub_agent_run_success(self):
        ...
        agent.llm.chat = AsyncMock(return_value=fake_response)

        result = await agent.run("test task")
        assert result == {"result": "success", "score": 42}

    @pytest.mark.asyncio
    async def test_sub_agent_run_with_context(self):
        from app.agents.sub_agent import SubAgent, SubAgentConfig
        config = SubAgentConfig(name="TestAgent", system_prompt="You are a test agent.")
        agent = SubAgent(config)
    ...
    @pytest.mark.asyncio
    async def test_sub_agent_run_strips_markdown_code_block(self):
        from app.agents.sub_agent import SubAgent, SubAgentConfig
        config = SubAgentConfig(name="TestAgent", system_prompt="You are a test agent.")
        agent = SubAgent(config)
    ...
    @pytest.mark.asyncio
    async def test_sub_agent_run_invalid_json_returns_raw(self):
        from app.agents.sub_agent import SubAgent, SubAgentConfig
        config = SubAgentConfig(name="TestAgent", system_prompt="You are a test agent.")
        agent = SubAgent(config)
...
class TestMCPServer:
    ...
    @pytest.mark.asyncio
    async def test_call_tool_with_api_key(self):
        from app.mcp.server import MCPServer
    ...
    @pytest.mark.asyncio
    async def test_call_tool_failure_returns_error_dict(self):
        from app.mcp.server import MCPServer
        server = MCPServer("test-server", "http://localhost:9000")
    ...
    @pytest.mark.asyncio
    async def test_list_tools_failure_returns_empty_list(self):
        from app.mcp.server import MCPServer
        server = MCPServer("test-server", "http://localhost:9000")
...
class TestMCPRegistry:
    ...
    def test_unregister_missing_does_not_raise(self):
        from app.mcp.server import MCPRegistry
        registry = MCPRegistry()
        registry.unregister("nonexistent")

    @pytest.mark.asyncio
    async def test_call_tool_server_found(self):
        from app.mcp.server import MCPRegistry
        registry = MCPRegistry()
        registry.register("my-server", "http://localhost:9000")

        with patch.object(registry.get("my-server"), "call_tool", AsyncMock(
    ...
    def test_list_servers_populated(self):
        from app.mcp.server import MCPRegistry
        registry = MCPRegistry()
