...
class _MinimalAgent(BaseAgent):
    ...
    async def _run(self, action: str, params: dict, session_id: str, trace_id: str):
...
class TestLoadPrompts:
    def test_matching_key_overrides_system_prompt(self):
        with (
            patch("builtins.open", MagicMock()),
            patch("app.agents.base.yaml.safe_load", return_value={"test": {"system": "Custom override"}}),  # noqa: E501
    ...
    def test_missing_key_keeps_default(self):
        with (
            patch("builtins.open", MagicMock()),
            patch("app.agents.base.yaml.safe_load", return_value={"other_agent": {"system": "Nope"}}),  # noqa: E501
            patch("app.agents.base.LLMClient"),
            patch("app.agents.base.get_hitl_gateway"),
        ):
            a = _MinimalAgent()
    ...
    def test_file_not_found_caught_silently(self):
        with (
    ...
    def test_invalid_yaml_caught_silently(self):
        with (
            patch("builtins.open", MagicMock()),
...
class TestEffectiveModel:
    ...
    def test_env_override_takes_precedence(self, agent):
    ...
    def test_empty_override_falls_back_to_model(self, agent):
        mock_settings = MagicMock()
...
class TestExecute:
    async def test_happy_path_default_action(self, agent):
        result = await agent.execute({"params": {"prompt": "hello"}}, session_id="s1", trace_id="t1")  # noqa: E501
        assert result["success"] is True
    ...
    async def test_happy_path_with_action(self, agent):
    ...
    async def test_rag_context_appended_when_present(self, agent):
    ...
    async def test_rag_context_skipped_when_empty(self, agent):
        with patch.object(agent, "_retrieve_context", AsyncMock(return_value="")):
            result = await agent.execute({"action": "analyze", "params": {"prompt": "test"}})
            assert result["success"] is True
            assert "rag_context" not in result["result"]["params"]

    async def test_re_raises_hitl_pending_error(self, agent):
    ...
    async def test_agent_error_returns_error_dict(self, agent):
    ...
    async def test_llm_unavailable_returns_degraded(self, agent):
    ...
    async def test_unexpected_exception_returns_error(self, agent):
...
class TestRetrieveContext:
    @pytest.mark.asyncio
    async def test_empty_results_returns_empty_string(self, agent):
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
            mock_vs.search = AsyncMock(return_value=[])
            mock_get.return_value = mock_vs
            result = await agent._retrieve_context("sess-1", {"action": "test", "params": {"prompt": "hi"}})  # noqa: E501
            assert result == ""

    @pytest.mark.asyncio
    async def test_results_with_content_returns_joined_context(self, agent):
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_results_with_missing_content_returns_empty(self, agent):
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_exception_is_caught_returns_empty(self, agent):
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_content_truncated_to_500_chars(self, agent):
        long_content = "x" * 1000
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_single_result_no_separator_needed(self, agent):
        with patch("app.memory.vector_store.get_vector_store") as mock_get:
            mock_vs = AsyncMock()
...
class TestLLMCall:
    @pytest.mark.asyncio
    async def test_llm_call_returns_content(self, agent):
    ...
    @pytest.mark.asyncio
    async def test_llm_call_default_temperature(self, agent):
...
class TestLLMCallRouted:
    @pytest.mark.asyncio
    async def test_returns_content(self, agent):
        agent.llm.chat_with_model_selection = AsyncMock(
            return_value=MagicMock(content="routed response")
        )
        result = await agent._llm_call_routed("code", [{"role": "user", "content": "write code"}], temperature=0.3)  # noqa: E501
    ...
    @pytest.mark.asyncio
    async def test_default_temperature(self, agent):
        agent.llm.chat_with_model_selection = AsyncMock(
            return_value=MagicMock(content="routed")
        )
...
class TestRequireHITL:
    @pytest.mark.asyncio
    async def test_requests_approval_with_correct_args(self, agent):
    ...
    @pytest.mark.asyncio
    async def test_passes_through_hitl_error(self, agent):
